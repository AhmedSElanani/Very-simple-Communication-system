...
%Fvec = linspace(min(min(Y)),max(max(Y)),length(Y));
Ymag = abs(Y);
figure;
plot(Fvec,Ymag)
xlabel('Frequency')
...
%%  Our Channel of choice is (Delta Function) 
%   Because the output in this case is exactly the same as the input

%   Create Channel one
Delta_ch = [1];
%   Perform Convolution between the input Audio file and the impulse
%   response of the channel to get the output of the channel
y = conv(Delta_ch,y);     %   Convolution in time domain 
%{
    OR -- >    Convolution in frequency Domain
...
xlabel('Frequency')
ylabel('Output Audio Signal')
title(' Channel''s Output''s Magnitude Spectrum ')
grid;
%   Play the audio file
x = y;
sound(x,Fs);
